...
def format_row(row, fields):
    row_data = {field: getattr(row, field, None) for field in fields if hasattr(row, field)}
...
def get_dates():
    ...
    dates = sorted({row.timestamp.strftime('%Y-%m-%d') for row in rows if row.timestamp})
...
def get_stress_levels():
    ...
    query = "SELECT DISTINCT device_id FROM environmental_data WHERE state = %s ALLOW FILTERING;"
    device_rows = session.execute(query, (state,))
...
def get_weather():
    ...
    if not temp_df.empty and not hum_df.empty:
        merged_df = pd.merge(temp_df, hum_df, on="timestamp", how="outer").fillna(method='ffill').fillna(method='bfill')
    else:
